...
class Orders extends Template
{
    ...
    {
        $wesupplyUrl = $this->helper->getWesupplyFullDomain();

        if ($this->isFirstLoginAttempt()) {

            if (!$this->getAuthSearchBy()) {
                return $this->_response->setRedirect($this->_storeManager->getStore()->getBaseUrl() . 'tracking-info');
            }

            $searchByKey = (array_keys($this->getAuthSearchBy()))[0];
            $searchByVal = ($this->getAuthSearchBy())[$searchByKey];

            return $wesupplyUrl . '?token=' . $this->getAuthToken() . '&' . $searchByKey . '=' . $searchByVal . '&platformType=' . $this->helper->getPlatform();
        }

        return $wesupplyUrl . 'account/orders?platformType=' . $this->helper->getPlatform();
    }
